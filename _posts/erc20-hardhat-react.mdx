---
date: '2021-06-20'
title: Tutorial - Create an ERC-20 token, deploy to rinkeby testnet and frontend to vercel
description: Create an ERC-20 token using hardhat, ethers and deploy to a live testnet and frontend to vercel
---

![alt text](/assets/erc20/yogacoin.png "ERC-20 YogaCoin")
...
> Code is available at [Github](https://github.com/ranadeep47/erc20-hardhat-waffle-react)

This project uses open-zepplin for smart contracts, hardhat, ethers for interacting with blockchain and waffle for writing tests and typescript, react for frontend, for UI we will use a UI library called baseweb to get us started quickly.
...
![alt text](/assets/erc20/local-hardhat-node.png "Starting local hardhat node")

To deploy our contract to hardhat's local network, we first create a folder called `build` and write a `deploy.js` script.
### Deploy script
``` javascript
...
![alt text](/assets/erc20/deploy-localhost.png "Deploying contract to local node")
...
![alt text](/assets/erc20/local-node-metamask.png "Adding local node to metamask")

Now in an other terminal window while the node is running, run `npm run start` and make sure you have `react-scripts start` in your `scripts` field in package.json. Now head to `http://localhost:3000` to interact with the contract, make sure you are connect to the Localhost Hardhat node network in your metamask wallet


---

## 4. Deploying contract to Rinkeby testnet
Deploying contract's requires Gas, so we require test ether from Rinkeby's faucet [here](https://www.rinkeby.io/#faucet). Make sure your metamask account is connected to `Rinkeby Test network` and check if it has some ether in it. 
...
Then create a `.env` file in your repo and add the following keys to it, make sure .env is file added in your `.gitignore`
...
![alt text](/assets/erc20/deploy-rinkeby.png "Deploying contract to rinkeby test net") 
You can see the address where our contract is deploy. To confirm it, lets head to etherscan and paste our contract address in the search bar. 

You should find the contract deployed on etherscan, with the tokens name, owner, balance etc..
![alt text](/assets/erc20/etherscan.png "Verifying contract on etherscan") 
...
![alt text](/assets/erc20/deploy-vercel.png "Deploying the frontend in vercel") 
